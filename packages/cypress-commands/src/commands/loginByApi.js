import { getPublicValue, setPublicValue } from '../helper/envHelpers.js'
 ...
 * the login instructions are stored as a public config value
...
Cypress.Commands.add('loginByApi', ({ username, password, baseUrl }) => {
    ...
    const hasApiAuthLogin = getPublicValue(HAS_API_AUTH_LOGIN_ENV_KEY)
    ...
    if (hasApiAuthLoginUnknown) {
        return cy.request(apiAuthLoginOptions).then((response) => {
            if (response.status === 404 || response.status === 302) {
                return cy.request(legacyLoginOptions).then((legacyResponse) => {
                    if (legacyResponse.status === 200) {
                        setPublicValue(HAS_API_AUTH_LOGIN_ENV_KEY, false)
            ...
            if (response.status === 200) {
                setPublicValue(HAS_API_AUTH_LOGIN_ENV_KEY, true)
